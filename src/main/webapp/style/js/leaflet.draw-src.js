...
L.Draw.Polyline = L.Draw.Feature.extend({
	...
	options: {
		...
		metric: true, // 是否使用公制测量系统或英制
		showLength: true, // 是否在工具提示中显示距离
		zIndexOffset: 2000 //这应该>任何地图图层的最高z-index
	},
	...
	_updateFinishHandler: function () {
		...
		// 最后一个标记应该有一个点击处理程序来关闭折线
		if (markerCount > 1) {
			this._markers[markerCount - 1].on('click', this._finishShape, this);
		}

		// 删除旧的标记点击处理程序（因为只有最后一个点应该关闭折线）
		if (markerCount > 2) {
...
L.Draw.Marker = L.Draw.Feature.extend({
	...
	options: {
		...
		repeatMode: false,
		zIndexOffset: 2000 //这应该>最高的z-index任何标记
	},

	initialize: function (map, options) {
		//保存类型触发，需要这样做，因为不能做这个。TYPE :(
...
L.EditToolbar.Edit = L.Handler.extend({
	...
	_enableLayerEdit: function (e) {
		var layer = e.layer || e.target || e,
		 ...
		 /*如果此图层是标记，但没有图标，则不要执行任何操作。 
		 标记通常应该有图标。 
		 如果在Leaflet.markercluster中使用Leaflet.draw，那么标记不会出现。*/
